import { totalDistance, setDifference, intersects } from "../helpers";
...
export const branchAndBoundOnCostAndCross = async (
  points: IPoint[],
  path: IPoint[] = [],
  visited: IPoint[] = [],
  ...
  if (path.length > 3) {
    ...
    for (let i = 1; i < path.length - 2; i++) {
      if (intersects(path[i], path[i - 1], newSegment[0], newSegment[1])) {
